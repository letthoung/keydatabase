...
function check_login($dbc, $user = '', $pass = '')
{
	...
	{ // If everything's OK.
		...
		if (mysqli_num_rows($r) == 1)
		{
			...
			if($row['pass'] !== null && password_verify($p, $row['pass']))
			{
			...
			else
			{
				if($row['pass'] == null)
				{
					$errors[] = 'Contact Work Control at 5660 to reset your password.';
				}
				else
				{
					$errors[] = 'The user id and password entered do not match those on file, you have tried to reset too many times.';
				}
...
function recordTimestamp($firstname, $lastname, $action){
	require("includes/mysqli_connect.php");

	date_default_timezone_set("America/Kentucky/Louisville");
	$current_time =  date ('Y-m-d H:i:s');
	//$action = 'Key Transfer from '.$ofirstname.' '.$olastname.' to '.$rfirstname.' '.$rlastname;
	$timestamp_query = "INSERT INTO activity_timestamps(firstname, lastname, date, action) VALUES ('$firstname', '$lastname', '$current_time', '$action')";
